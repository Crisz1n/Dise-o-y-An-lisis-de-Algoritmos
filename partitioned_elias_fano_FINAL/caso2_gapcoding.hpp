...
- Ojo: el primer valor (GC[0]) no es una diferencia, es el numero real A[0] y puede ser gigante. Ese lo guardamos aparte en un 
...
struct Caso2 {
    ...
    void construir(const vector<uint64_t>& A, size_t b = 0) {
        ...
        //Armar el sample: guardamos A[0], A[salto], A[2*salto]... o sea los valores reales en posiciones espaciadas. 
    ...
    size_t espacio_bytes() const {
        return GC.size() * sizeof(uint32_t)
             + sample.size() * sizeof(uint64_t)
             + sizeof(uint64_t);
    }
};

//Misma medicion de tiempo que el Caso 1 
double medir_tiempo_busqueda_caso2(const Caso2& c2,
